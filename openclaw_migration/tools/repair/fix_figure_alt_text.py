...
Two modes:
  ...
  - manual: reads a JSON map of {mcid: alt_text} and applies exactly those
...
parser.add_argument('--alt-map', help='JSON file mapping figure MCID or index to alt text')
...
for page_num, page in enumerate(doc):
    ...
    # Walk structure tree for Figure elements via page annotations + struct
    for img_info in page.get_images(full=True):
        xref = img_info[0]
        # Check if this image has an associated structure element with Alt
        # PyMuPDF exposes struct via page.get_label / pdf_catalog
        # We use the lower-level approach: find Figure structs in the struct tree
        pass

# Walk PDF structure tree for Figure elements
...
def walk_struct(xref, doc, depth=0):
    ...
    try:
        ...
        yield (xref, s_type[1] if s_type[0] != 'null' else '', 
